...
export function AgendamentoScreen({ onBack }) {
  const [dataVisualizacao, setDataVisualizacao] = useState(new Date()); // Data para navegar no calendário
  ...
  // --- REGRA: LIMPEZA AUTOMÁTICA (Remover agendamentos com mais de 1 semana) ---
  useEffect(() => {
    const hoje = new Date();
  ...
  // --- REGRA: BLOQUEIO DE DIAS ANTERIORES NO SALVAMENTO ---
  const handleSalvarAgendamento = (e) => {
    ...
    if (dataEscolhida < hoje) {
      alert("⚠️ Não é possível realizar agendamentos em datas passadas.");
      return;
    }
      ...
      <header className="app-header">
        <div className="header-left">
          ...
          <div>
            ...
            <div className="app-sm">Regras de negócio e OS vinculadas.</div>
          </div>
        </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(320px, 1fr))", gap: "30px" }}>
        ...
        <div className="section" style={{ background: "rgba(255,255,255,0.02)", padding: "20px", borderRadius: "20px", border: "1px solid var(--border)" }}>
          ...
          <form onSubmit={handleSalvarAgendamento} style={{ marginTop: "25px" }}>
            <div className="section-title" style={{ marginBottom: "10px" }}>Agendar para: {diaSelecionado}/{mesAtual + 1}</div>
            ...
            <button type="submit" className="btn-primary" style={{ width: "100%", marginTop: "15px" }}>CONFIRMAR AGENDAMENTO</button>
          </form>
        </div>

        <div>
          <div className="section-title">Serviços Ativos</div>
          <div style={{ display: "flex", flexDirection: "column", gap: "12px", maxHeight: "600px", overflowY: "auto", paddingRight: "5px" }}>
            {agendamentosDoMes.length === 0 ? (
              <div className="app-sm" style={{ textAlign: "center", padding: "40px" }}>Nenhum serviço registrado.</div>
            ) : (
              agendamentosDoMes.map(ag => (
                <div key={ag.id} className="item-card" style={{ borderLeft: ag.dia === diaSelecionado ? "4px solid var(--accent)" : "1px solid var(--border)" }}>
                  ...
                  <div className="item-actions">
                    {ag.pdf && (
                      <button className="btn-icon" onClick={() => abrirPdf(ag.pdf)}>📄</button>
                    )}
                    <button className="btn-icon" onClick={() => {
                       if(confirm("Deseja excluir este agendamento?")) setAgendamentos(agendamentos.filter(x => x.id !== ag.id))
